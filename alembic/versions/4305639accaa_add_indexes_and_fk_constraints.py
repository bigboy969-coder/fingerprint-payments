...
down_revision: str | Sequence[str] | None = "9db10166ac38"
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_verification_codes_lookup", "customer_verification_codes", ["email", "code", "used"]
    )
    ...
    with op.batch_alter_table("fingerprints") as batch_op:
        batch_op.create_foreign_key(
            "fk_fingerprints_user", "users", ["user_id"], ["id"], ondelete="CASCADE"
        )

    with op.batch_alter_table("transactions") as batch_op:
        batch_op.create_foreign_key("fk_transactions_user", "users", ["user_id"], ["id"])
        batch_op.create_foreign_key(
            "fk_transactions_merchant", "merchants", ["merchant_id"], ["id"]
        )
